...
export default function RentPage() {
  ...
  const roomOptions = [
    {key: "1", label: t("rent.filters.room1") || "1 кімната"},
    {key: "2", label: t("rent.filters.room2") || "2 кімнати"},
    {key: "3", label: t("rent.filters.room3") || "3 кімнати"},
    {key: "4", label: t("rent.filters.room4") || "4 кімнати"},
    {key: "5", label: t("rent.filters.room5plus") || "5+ кімнат"},
  ];

  const floorOptions = [
    {key: "1", label: t("rent.filters.floor1") || "1 поверх"},
    {key: "2", label: t("rent.filters.floor2") || "2 поверх"},
    {key: "3", label: t("rent.filters.floor3") || "3 поверх"},
    {key: "4", label: t("rent.filters.floor4") || "4 поверх"},
    {key: "5", label: t("rent.filters.floor5plus") || "5+ поверх"},
  ];
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const response = await fetch('/api/properties?transaction_type=rent');
  ...
  useEffect(() => {
    ...
    if (filterCategory !== "all") {
      if (filterCategory === "daily") {
        ...
        result = result.filter((property) => property.price > 1000 || (property.tags && Array.isArray(property.tags) && (property.tags.includes("преміум") || property.tags.includes("VIP"))));
      }
    }
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            >
{t("rent.filters.searchButton") || "Шукати"}
            </Button>
          </div>
            ...
            {(onClose) => (
              <>
                <DrawerHeader className="flex flex-col gap-1">
                  <h2 className="text-xl font-semibold">{t("rent.filters.searchTitle") || "Фільтри пошуку"}</h2>
                  <p className="text-sm text-gray-500">{t("rent.filters.searchSubtitle") || "Налаштуйте параметри для пошуку нерухомості в оренду"}</p>
                </DrawerHeader>
                <DrawerBody>
                  <div className="space-y-6">
                    {/* Категорія оренди */}
                    <div>
                      <label className="block text-sm font-medium mb-2">{t("rent.filters.rentalCategory") || "Категорія оренди"}</label>
                      <Autocomplete
                        defaultItems={rentCategories}
                        placeholder={t("rent.filters.rentalCategoryPlaceholder") || "Оберіть категорію"}
                    ...
                    {/* Кількість кімнат */}
                    <div>
                      <label className="block text-sm font-medium mb-2">{t("rent.filters.rooms") || "Кількість кімнат"}</label>
                      <Autocomplete
                        defaultItems={roomOptions}
                        placeholder={t("rent.filters.roomsPlaceholder") || "Оберіть кількість кімнат"}
                    ...
                    {/* Поверх */}
                    <div>
                      <label className="block text-sm font-medium mb-2">{t("rent.filters.floor") || "Поверх"}</label>
                      <Autocomplete
                        defaultItems={floorOptions}
                        placeholder={t("rent.filters.floorPlaceholder") || "Оберіть поверх"}
                    ...
                    {/* Ціна за місяць */}
                    <div>
                      <label className="block text-sm font-medium mb-2">{t("rent.filters.price") || "Оренда на місяць"} ({currency})</label>
                      <div className="grid grid-cols-2 gap-3">
                        <Input
                          placeholder={t("rent.filters.from") || "Від"}
                          value={priceMin}
                        ...
                        />
                        <Input
                          placeholder={t("rent.filters.to") || "До"}
                          value={priceMax}
                    ...
                    {/* Площа */}
                    <div>
                      <label className="block text-sm font-medium mb-2">{t("rent.filters.area") || "Площа"} (м²)</label>
                      <div className="grid grid-cols-2 gap-3">
                        <Input
                          placeholder={t("rent.filters.from") || "Від"}
                          value={areaMin}
                ...
                <DrawerFooter>
                  ...
                  >
                    {t("rent.filters.apply") || "Застосувати"}
                  </Button>
                </DrawerFooter>
              </>
            )}
